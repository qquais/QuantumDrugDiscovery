...
class Generator(nn.Module):
    """Generator network of MolGAN (original architecture used in your training)"""

    def __init__(self, conv_dims, z_dim, vertexes, edges, nodes, dropout_rate):
        super(Generator, self).__init__()
        self.conv_dims = conv_dims           # e.g. [128]
        self.z_dim = z_dim                   # 8
        self.vertexes = vertexes             # 9
        self.edges = edges                   # 5 bond types
        self.nodes = nodes                   # 6 atom types
        self.dropout_rate = dropout_rate

        self.activation_f = nn.Tanh()
        self.multi_dense_layers = MultiDenseLayers(z_dim, conv_dims, self.activation_f)

        # OUTPUT SIZES USED DURING TRAINING:
        # edges: 5 × 9 × 9 = 405
        # nodes: 9 × 6 = 54
        self.edges_layer = nn.Linear(conv_dims[-1], self.edges * self.vertexes * self.vertexes)
        self.nodes_layer = nn.Linear(conv_dims[-1], self.vertexes * self.nodes)
    ...
    def forward(self, x):
        ...
        # edges: (batch, edges, V, V)
        edges_logits = self.edges_layer(output).view(-1, self.edges, self.vertexes, self.vertexes)

        # make adjacency symmetric
        edges_logits = (edges_logits + edges_logits.permute(0, 1, 3, 2)) / 2

        # final shape: (batch, V, V, edges)
        edges_logits = self.dropout(edges_logits.permute(0, 2, 3, 1))

        # nodes: (batch, V, nodes)
        nodes_logits = self.nodes_layer(output).view(-1, self.vertexes, self.nodes)
