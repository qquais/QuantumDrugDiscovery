"""DEPRECATED — the v1 training loop, kept only as a reference for the audit.

Do not run this. It contains the defects documented in docs/ERRATA.md:
training-time metrics computed on batches of 16, normalised SA/logP reported
as property values, property means taken over np.nonzero(), circuit weights
appended positionally to a CSV, and checkpoints saved twice per epoch. It also
imports `frechetdist` and `experimental/q_discriminator.py`, so it may not even
import in the current environment. The maintained loop is `solver.py`.
"""
